...
func getExampleList(engine: PaylikeEngine) -> [Example] {
    let examples: [Example] = [
        Example(id: "1", title: "Paylike Style example", details: "A custom-made widget with Paylike styling boosts customer trust in your application\'s payment flow. This widget has the same capabilities as the Simple White Label form but uses Paylike theme and predefined Paylike style UI elements.", view: AnyView(SimplePaylikeExample(engine: engine))),
        Example(id: "2", title: "Example with extended data", details: "Extended data is added to payment as text, and custom data", view: AnyView(ExtendedPaylikeExample(engine: engine))),
